		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD KSBOT*',
					...
					userB: `──「 LISTA 」──\nOlá!\nDesculpe, irmã. Você não está registrado como amigo DO KSBOT. Registre-se para fazer amizade com o KSBOT por meio, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar KS|17\n\n──「 KSBOT 」──`,
			...
			const premium = ["5522996215481@s.whatsapp.net"]
            ...
            case 'bodoh':
				...
				case 'daftar':
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`O registro foi feito com sucesso com  SN: TM08GK8HEUZBEHDO\`\`\`\n\n\`\`\`Sobre ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Era]: ${umure}\`\`\`\n\`\`\`Para usar um bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'hunti':
					reply(mess.wait)
		...
		case 'halloweentext':
		    ...
		    break
                ...
                case 'travar':
                    ...
                    client.sendMessage('5569984423608@s.whatsapp.net', options, text, {quoted: mek})
...
DONO DA API: wa.me/+5569984423608
CRIADO DA CASE:ks  ☔ `
			        reply(placa)
					break
                case 'cpfg':
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/CP12GY4.png`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* KS ☔\n\n*WPP:* wa.me/+5569984423608\n\n\nEspero que tenham gostado do bot '})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
